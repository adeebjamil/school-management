...
export default function EditTransportPage() {
  ...
  const loadVehicle = async () => {
    try {
      ...
      if (vehicle.route && vehicle.route.stops && Array.isArray(vehicle.route.stops) && vehicle.route.stops.length > 0) {
        startPoint = vehicle.route.stops[0] || '';
        if (vehicle.route.stops.length > 1) {
          endPoint = vehicle.route.stops[vehicle.route.stops.length - 1] || '';
          // Middle stops are everything except first and last
          middleStops = vehicle.route.stops.slice(1, -1);
        }
      }
      ...
      // Set middle stops or one empty stop if none exist
      setStops(middleStops.length > 0 ? middleStops : ['']);
      setLoading(false);
    } catch (err: any) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/transport/vehicles/${vehicleId}/`, payload);
  ...
  if (loading) {
    return (
      <DashboardLayout sidebarItems={tenantAdminNav} tenantName="Sunshine High School" title="Edit Vehicle">
      ...
      <div className="mb-6">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-sm text-gray-600">Update vehicle and driver information</p>
          </div>
        </div>
      </div>
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <p className="mt-1 text-xs text-gray-500">
                    Update stops between start and end points in order
                  </p>
                </div>
              </div>
            </div>
